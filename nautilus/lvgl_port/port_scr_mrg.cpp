...
static scr_card_t *scr_mgr_get_top(scr_card_t *head) 
{
    scr_card_t *p = head;
...
static void scr_mgr_active(scr_card_t *card)
{
    if(card->st == SCR_MGR_STATE_DESTROYED || card->obj == NULL){
...
bool scr_mgr_push(int id, bool anim)
{
    ...
    if((scr_stack_top != NULL) && (tgt_card->id == scr_stack_top->id)){
        return false;
    }
